...
El precio de usar la API de OpenAI se cobra por cada token procesado, tanto de entrada (lo que escribes) como de salida (lo que responde la IA). Además, los modelos tienen un límite máximo de tokens que pueden aceptar y recordar a la vez (ventana de contexto). Por eso es importante que utilicemos antes el tokenizador y eliminemos la información que estemos aportando al modelo que no sea necesaria. Por ejemplo, saludar al chat o despedirnos de él.
